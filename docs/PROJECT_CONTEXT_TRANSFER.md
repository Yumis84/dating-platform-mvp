...
## 2026-08-12 — Real n8n 2.34.5 runtime compatibility corrective
...
- Real n8n exposed a repository JSON compatibility issue that static/PGlite tests could not: `If` typeVersion 2 nodes were stored in legacy `conditions.boolean` / `conditions.string` form. They are now stored using the v2 filter contract. Converted WF_01 nodes: Should Handoff WOMAN Event, Is Role Choice, Is MAN Name Confirmation, Is MAN Catalog Choice, Has Callback Query; converted WF_03 nodes: WOMAN Session Available, Is Photo, WOMAN Profile Complete.
