...
|------|-------------------|
| **Мужчина** | Подтверждённое имя, обязательный город каталога, необязательные ranking-preferences, каталог, избранное, чат. Публичная `profiles`-анкета не создаётся. |
| **Женщина** | WOMAN-only AI onboarding по ТЗ №2: анкета, фото, цены, места встречи, модерация. |
...
- MAN и WOMAN onboarding — разные data models и workflow branches;
- город — единственный hard filter каталога и сравнивается по нормализованному значению;
- NULL MAN preference не влияет на score и не исключает WOMAN profile.
