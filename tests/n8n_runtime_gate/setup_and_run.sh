...
cleanup() {
  ...
  if [[ -f "$MOCK_LOG" ]]; then cp "$MOCK_LOG" "$ARTIFACT_DIR/mock-ai.log"; fi
  if [[ -f "$GATE_DIR/gate-report.json" ]]; then cp "$GATE_DIR/gate-report.json" "$ARTIFACT_DIR/gate-report.json"; fi
...
for _ in $(seq 1 30); do
  if curl --silent --fail http://127.0.0.1:18080/healthz >/dev/null 2>&1; then break; fi
  sleep 1
done
...
import json,sys
print(json.dumps({'email':'runtime-gate@example.invalid','firstName':'Runtime','lastName':'Gate','password':sys.argv[1]}))
PY
)" >/dev/null
...
printf '\n== Strict successful WF_01 -> WF_03 UX probe ==\n'
python tests/n8n_runtime_gate/strict_success_handoff.py
