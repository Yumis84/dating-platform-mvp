...
This section is append-only and supplements, rather than replaces, the historical context above.

- MAN: explicit resumable name-confirmation/manual-name/city state; no public profile; preferences optional.
- WOMAN: idempotent DRAFT profile plus IN_PROGRESS session; WF_01 to WF_03 payload includes Telegram/user/profile/session identifiers and text or photo file_id.
- WF_03 structured collections use explicit APPEND/UPDATE/DELETE operations; silent position overwrite is forbidden. Photo insertion is serialized per profile and deduplicated by Telegram file_id.
- Catalog ranking keeps the candidate set constrained only by ACTIVE + WOMAN + equal normalized city.
- Files 009/010 and WF_01/WF_03/WF_05 JSON are repository drafts only. Do not apply, import, activate, publish, or merge without separate approval.
